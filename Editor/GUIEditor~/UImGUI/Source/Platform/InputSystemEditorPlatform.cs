...
namespace Kit.UIMGUI.Platform
{
	...
	/// </summary>
	internal sealed class InputSystemEditorPlatform : PlatformBase
	{
		...
		public InputSystemEditorPlatform(CursorShapesAsset cursorShapes, IniSettingsAsset iniSettings) :
		...
		private static unsafe void UpdateMouse(ImGuiIOPtr io, Mouse mouse)
		{
			...
			if (Application.isPlaying)
			{
				...
				Vector2 viewportMousePos = new Vector2(mousePos.x / SceneView.lastActiveSceneView.camera.pixelWidth, 1.0f - (mousePos.y / SceneView.lastActiveSceneView.camera.pixelHeight));
				 
				mousePos = new Vector2(viewportMousePos.x * Screen.width, (0.0f + viewportMousePos.y) * Screen.height);
				MouseState state = new MouseState()
				{
					position = mousePos,
					scroll = Event.current.isScrollWheel ? Event.current.delta : Vector2.zero,
					clickCount = (ushort)Event.current.clickCount,
					delta = Event.current.delta,
					buttons = (ushort)Event.current.button,
					displayIndex = (ushort)Event.current.displayIndex
				}; 
